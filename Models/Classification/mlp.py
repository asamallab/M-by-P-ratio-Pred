...
mlp = MLPClassifier()
mlp_parameters = {
    "hidden_layer_sizes":[(3,), (5,), (7,), (10,), (20,), (50,),(100,)],
...
mlp_model = GridSearchCV(mlp, mlp_parameters, cv=cv).fit(train, Train_target)
print(mlp_model.best_score_, " ", mlp_model.best_params_)


cv_df = pd.DataFrame(mlp_model.cv_results_)
cv_df.to_csv(others + "/CV_results.csv", sep="\t")

best_params = pd.DataFrame(mlp_model.best_params_, index=[0])
best_params.to_csv(others + "/best_params.csv", sep="\t")

# MLP model Generation and Training
mlpc_model = MLPClassifier(
    hidden_layer_sizes=mlp_model.best_params_["hidden_layer_sizes"],
    activation=mlp_model.best_params_["activation"],
    solver=mlp_model.best_params_["solver"],
    learning_rate=mlp_model.best_params_["learning_rate"],
    early_stopping=mlp_model.best_params_["early_stopping"],
    max_iter=mlp_model.best_params_["max_iter"],
    random_state=seed,
).fit(train, Train_target)
mlp_train_accuracy = mlpc_model.score(train, Train_target)
print(mlp_train_accuracy)
pickle.dump(mlpc_model, open(resultpath + "/" + model, "wb"))

train_dict = {"Model": model, "Topk": topk}
test_dict = {"Model": model, "Topk": topk}

mlp_pred_train = mlpc_model.predict(train)
mlp_train_tn, mlp_train_fp, mlp_train_fn, mlp_train_tp = confusion_matrix(
    Train_target, mlp_pred_train
).ravel()
print(
    "TN: ",
    mlp_train_tn,
...
(
    ...
    mlp_train_fscore,
    mlp_train_support,
) = precision_recall_fscore_support(Train_target, mlp_pred_train, average="binary")
print(
    "Precision: ",
    mlp_train_precision,
    " ",
...
mlp_train_accuracy = accuracy_score(Train_target, mlp_pred_train)
print("Accuracy: ", mlp_train_accuracy)

conf_matrix = confusion_matrix(y_true=Train_target, y_pred=mlp_pred_train)
#
...
train_dict["Accuracy"] = mlp_train_accuracy
train_dict["TN"] = mlp_train_tn
train_dict["FP"] = mlp_train_fp
train_dict["FN"] = mlp_train_fn
train_dict["TP"] = mlp_train_tp

train_dict["Precision"] = mlp_train_precision
train_dict["Recal"] = mlp_train_recal
train_dict["fscore"] = mlp_train_fscore

mlp_pred = mlpc_model.predict(test)
mlp_tn, mlp_fp, mlp_fn, mlp_tp = confusion_matrix(Test_target, mlp_pred).ravel()
print("TN: ", mlp_tn, "FP: ", mlp_fp, "FN: ", mlp_fn, "TP: ", mlp_tp)
mlp_precision, mlp_recal, mlp_fscore, mlp_support = precision_recall_fscore_support(
    Test_target, mlp_pred, average="binary"
)
print(
    "Precision: ",
    mlp_precision,
...
mlp_test_accuracy = accuracy_score(Test_target, mlp_pred)
print("Accuracy: ", mlp_test_accuracy)

conf_matrix = confusion_matrix(y_true=Test_target, y_pred=mlp_pred)
#
# Print the confusion matrix using Matplotlib
#
fig, ax = plt.subplots(figsize=(7.5, 7.5))
...
test_dict["Accuracy"] = mlp_test_accuracy
test_dict["TN"] = mlp_tn
test_dict["FP"] = mlp_fp
test_dict["FN"] = mlp_fn
test_dict["TP"] = mlp_tp

test_dict["Precision"] = mlp_precision
test_dict["Recal"] = mlp_recal
test_dict["fscore"] = mlp_fscore
...
Roc_prob = mlpc_model.predict_proba(test)
Roc_prob = Roc_prob[:, 1]

print("shapes ", Test_target.shape," ",Roc_prob.shape)
mlp_auc = roc_auc_score(Test_target, Roc_prob)
print('MLP: ROC AUC=%.3f' % (mlp_auc))

mlp_fpr, mlp_tpr, _ = roc_curve(Test_target, Roc_prob)
...
plt.plot(mlp_fpr, mlp_tpr, label="AUC="+str(round(mlp_auc, 4)))
# axis labels
plt.xlabel('1 - Specificity')
plt.ylabel('Sensitivity')
# show the legend
plt.legend(loc=4)
plt.title("MLP ROC", fontsize=18)
plt.savefig(plotpath + "/ROC_MLP.png")
plt.savefig(plotpath + "/ROC_MLP.pdf")
plt.close()

######## PR Curve ############
mlp_precision, mlp_recall, mlp_thresholds = precision_recall_curve(Test_target, Roc_prob)
print(len(mlp_thresholds))
mlp_auprc = auc(mlp_recall, mlp_precision)

mlp_avg_precision = average_precision_score(Test_target, Roc_prob)
print("MLP", mlp_auprc," ",mlp_avg_precision)
plt.plot(mlp_recall, mlp_precision, label='MLP = %0.2f' % mlp_auprc)
